...
public class MetaDataExtractor {
	...
	public static void main(String[] args) throws ClassNotFoundException, IDMapperException {
		if(args.length == 5) {
			...
			//file = "pathways/WP10/WP10.gpml";
			//date = "2022-06-15T16:15:22Z";
			
			try {
				if(file.endsWith(".gpml")) {
					System.out.println("Found gpml file");
					URL url = new URL("https://raw.githubusercontent.com/" + repo + "/main/" + file);
					//url = new URL("https://raw.githubusercontent.com/wikipathways/wikipathways-database/main/pathways/WP10/WP10.gpml");
	...
	private static void printNodeList(String pId, Pathway pwy) throws IOException, ClassNotFoundException, IDMapperException {
		...
		w.write("Label\tType\tIdentifier\tComment\tDatabase\tEnsembl\tNCBI gene\tHGNC\tUniProt\tWikidata\tChEBI\tInChI\n");
		...
		for (String type : elementTypes){
			for(PathwayElement e : pwy.getDataObjects()) {
				if(e.getObjectType().equals(ObjectType.DATANODE) && e.getDataNodeType().equals(type)){
					...
					w.write(e.getTextLabel().replace("\n", "") + "\t" + e.getDataNodeType() + "\t" + ((e.getElementID() != null) ? e.getElementID() : "") + "\t" +  comment  + "\t" + ((e.getDataSource() != null) ? e.getDataSource().getFullName() : "") + "\t" + idMappings + "\n");			
				}
			}
		}
		w.close();
	...
	private static String getIDMappingsString(PathwayElement e, String pId, Pathway pwy, IDMapperStack idmpStack) throws ClassNotFoundException, IOException, IDMapperException {
		...
		Set<Xref> res = idmpStack.mapID(e.getXref(),DataSource.getExistingBySystemCode("En"),DataSource.getExistingBySystemCode("L"), 
				DataSource.getExistingBySystemCode("H"),DataSource.getExistingBySystemCode("S"),DataSource.getExistingBySystemCode("Wd")
				,DataSource.getExistingBySystemCode("Ce"),DataSource.getExistingBySystemCode("Ik"));
